...
import { BillingStatusBadge, type SubscriptionStatus, calculateTrialDaysRemaining } from './UI/BillingStatusBadge';
...
export function TopBar({ currentOrg, onOrgChange, onLogout, subscriptionStatus = 'TRIAL', trialEndsAt }: TopBarProps) {
  const [showOrgMenu, setShowOrgMenu] = useState(false);
  const [showUserMenu, setShowUserMenu] = useState(false);

  // Calculate trial days remaining
  const trialDaysRemaining = trialEndsAt ? calculateTrialDaysRemaining(trialEndsAt) : undefined;
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 z-50">
      <div className="h-full px-6 flex items-center justify-between">
        ...
        {/* Org Selector with Billing Status */}
              ...
              <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-2">
                {orgs.map((org) => (
                  <button
                    ...
                    className={`w-full px-4 py-2 text-right hover:bg-gray-50 ${org === currentOrg ? 'bg-blue-50 text-blue-600' : ''
                      }`}
                  >
                    {org}
                  </button>
