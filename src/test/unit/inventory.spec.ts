...
/**
 * TODO: These tests need refactoring to match current InventoryService API
 * - reserveStockForOrder now takes (orderId, organizationId) - 2 args
 * - releaseStockForOrder now takes (orderId, organizationId, reason) - 3 args
 * - adjustStock signature has changed significantly
 * - Tests use userId parameter that's no longer in some method signatures
 */
describe.skip('InventoryService - Unit Tests (NEEDS REFACTOR)', () => {
  let prisma: PrismaClient;
  let inventoryService: InventoryService;

  beforeAll(async () => {
    prisma = await setupTestDB();
    // Cast PrismaClient to PrismaService for service compatibility
    inventoryService = new InventoryService(prisma as unknown as PrismaService);
  });
