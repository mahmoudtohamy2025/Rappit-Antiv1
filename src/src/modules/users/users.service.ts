...
export class UsersService {
  ...
  async create(
    ...
    if (user) {
      ...
      user = await this.prisma.user.create({
        data: {
          ...
          isActive: false, // Start as pending until invite is accepted
        },
      });
    ...
    // Generate invite token if user is not active or needs invite
    let inviteToken = null;
    if (!user.isActive || !user.password) {
      inviteToken = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
      const inviteExpiresAt = new Date();
      inviteExpiresAt.setHours(inviteExpiresAt.getHours() + 24); // 24 hours expiry
      ...
      user = await this.prisma.user.update({
        ...
        data: {
          inviteToken,
          inviteExpiresAt,
          isActive: false, // Ensure user is inactive until they accept
        }
      });

      // Send invite email
      // In a real app, you would use a proper frontend URL config
      const inviteUrl = `http://localhost:3000/accept-invite?token=${inviteToken}`;
      const organization = await this.prisma.organization.findUnique({ where: { id: organizationId } });

      try {
        await this.emailService.sendInvite(user.email, inviteUrl, organization?.name || 'Rappit Organization');
      } catch (error) {
        this.logger.error(`Failed to send invite email to ${user.email}`, error);
   ...
   */
  async resendInvite(userId: string, organizationId: string, requestedByRole: UserRole) {
    ...
    try {
      ...
      this.logger.error(`Failed to resend invite to ${user.email}`, error);
    }

    return { message: 'Invite sent successfully', id: user.id };
  }
