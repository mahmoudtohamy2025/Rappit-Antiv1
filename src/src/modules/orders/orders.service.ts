...
export class OrdersService {
  ...
  async createOrUpdateOrderFromChannelPayload(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // 2. Prepare addresses (Store as JSON)
      ...
      const itemsWithSkuIds = await Promise.all(
        payload.items.map(async (item) => {
          const sku = await tx.sKU.findFirst({
            where: { sku: item.sku, organizationId }, // Changed to findFirst + ordId just in case
          });
      ...
      await tx.orderTimelineEvent.create({
        data: {
          orderId: order.id,
          organizationId,
          event: isNewOrder ? 'order_created' : 'order_updated',
          actorType,
          actorId,
          metadata: {
      ...
      if (order.status === 'NEW' || order.status === 'RESERVED') {
        try {
          ...
          // Update order status to RESERVED only if not already reserved
          if (order.status !== 'RESERVED') {
            return await this.updateOrderStatus(
              ...
              ActorType.SYSTEM,
              null,
              'Initial inventory reservation on order creation',
            );
          } else {
            // Return fresh order to ensure status is correct
            return this.findOne(organizationId, order.id);
          }
        } catch (error) {
          this.logger.error(
            `Failed to reserve inventory for order ${order.orderNumber}: ${error.message}`,
          );
          // Small delay to ensure DB transaction from concurrent request is visible
          await new Promise(resolve => setTimeout(resolve, 1000));
          // Return fresh order state (likely NEW or RESERVED depending on race)
          return this.findOne(organizationId, order.id);
        }
      }
  ...
  async updateOrderStatus(
    ...
    // Timestamp update removed as fields do not exist in schema
